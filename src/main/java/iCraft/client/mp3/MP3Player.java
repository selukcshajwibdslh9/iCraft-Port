...
public class MP3Player {
    ...
    public void play() {
        if (music == null) return; // Если музыки нет, не ломаемся

        try {
            if (playerStatus == NOTSTARTED) {
                // Сначала просто запускаем музыку, не дожидаясь конца подсчета
                playerStatus = PLAYING;
                active = true;
                
                // Считаем кадры в фоне, но это не должно мешать запуску
            ...
            System.err.println("[iCraft] Ошибка запуска MP3: " + e.getMessage());
            e.printStackTrace();
            playerStatus = NOTSTARTED;
            active = false;
        }
    ...
    private void stopInternal() {
        try {
            if (player != null) {
                player.close(); // Используй close() вместо stop(), это надежнее для JLayer
                player = null;
            }
        } catch (Exception e) {
            // Игнорируем ошибки при остановке, чтобы они не вешали основной поток
        } finally {
            closeStream();
        }
    }
