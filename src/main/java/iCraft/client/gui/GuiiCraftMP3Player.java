...
public class GuiiCraftMP3Player extends GuiiCraftBase {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTick, int mouseX, int mouseY) {
        ...
        if (ICraft.mp3Player != null && ICraft.mp3Player.hasPlayer()) {
            ...
            int displayInt = ICraft.mp3Player.getMusicStatus(68);
    ...
    protected void mouseClicked(int x, int y, int button) throws IOException {
        ...
        if (button == 0) {
            ...
            if (xAxis >= 80 && xAxis <= 95 && yAxis >= 143 && yAxis <= 158)
                mc.player.openGui(ICraft.instance, 0, mc.world, 0, 0, 0);
            // Slider
                ...
                if (getIndex() + i < ICraft.musics.size()) {
                    ...
                    if (xAxis >= 52 && xAxis <= 111 && yAxis >= yStart && yAxis <= yStart + 14) {
                        try {
                            ...
                            ICraft.mp3Player.setMusic(ICraft.musics.get(getIndex() + i).toURI().toURL());
                            ICraft.mp3Player.resetPlayerStatus();
                            ICraft.mp3Player.setRepeatType(ICraft.mp3Player.getRepeatType());
                            ICraft.mp3Player.play();

                            mc.player.sendMessage(new TextComponentString(TextFormatting.BLUE + "[" + TextFormatting.GOLD + "iCraft" + TextFormatting.BLUE + "] " + TextFormatting.GREEN + "Playing " + TextFormatting.DARK_PURPLE + ICraft.musicNames.get(getIndex() + i)));
                        } catch (Exception e) {
            ...
            if (ICraft.mp3Player != null && ICraft.mp3Player.hasPlayer() && ICraft.mp3Folder.listFiles().length != 0) {
                ...
                if (xAxis >= 103 && xAxis <= 112 && yAxis >= 122 && yAxis <= 129) {
                    ICraft.mp3Player.close();
                    try {
                        ICraft.mp3Player.setMusic(ICraft.musics.get((ICraft.currentMusicId + 1 > ICraft.musics.size() - 1 ? 0 : ICraft.currentMusicId + 1)).toURI().toURL());
                        ICraft.mp3Player.resetPlayerStatus();
                        ICraft.mp3Player.play();

                        mc.player.sendMessage(new TextComponentString(TextFormatting.BLUE + "[" + TextFormatting.GOLD + "iCraft" + TextFormatting.BLUE + "] " + TextFormatting.GREEN + "Playing " + TextFormatting.DARK_PURPLE + ICraft.musicNames.get(ICraft.currentMusicId)));
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    }
                }
                ...
                if (xAxis >= 64 && xAxis <= 73 && yAxis >= 122 && yAxis <= 129) {
                    ICraft.mp3Player.close();
                    try {
                        ICraft.mp3Player.setMusic(ICraft.musics.get((ICraft.currentMusicId - 1 < 0 ? ICraft.musics.size() - 1 : ICraft.currentMusicId - 1)).toURI().toURL());
                        ICraft.mp3Player.resetPlayerStatus();
                        ICraft.mp3Player.play();
