...
public class CommonProxy {
   public void loadConfiguration() {
      ...
      ICraft.buyableItems = ICraft.configuration.get("general", "Buyable Items(I know purchasable is more correct, but buyable is funnier :P)", new String[]{Item.getIdFromItem(Items.IRON_INGOT) + ".1.0-" + Item.getIdFromItem(Items.IRON_INGOT) + ".1.0", Item.getIdFromItem(Items.GOLD_INGOT) + ".1.0-" + Item.getIdFromItem(Items.GOLD_INGOT) + ".1.0", Item.getIdFromItem(Items.DIAMOND) + ".1.0-" + Item.getIdFromItem(Items.DIAMOND) + ".1.0"}, "To add or modify items of the CraftBay Shop use this pattern: id.quantity.metadata-id.quantity.metadata - And always remember, on the left side of the hyphen(the '-' symbol), put the items you want to be purchasable and on the right side, put items you want to use as payment for the left-side item.").getStringList();
